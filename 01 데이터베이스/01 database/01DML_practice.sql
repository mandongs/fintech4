`hongong 1`-- 데이터 베이스 목록 조회 show databases;
show databases;
...
select avg(rental_rate) as 대여요금 from film; -- 2.98 # 기존: 2문장
select title, rental_rate from film where rental_rate > 2.98;
# 서브쿼리: 1문장
select title, rental_rate from film where rental_rate > (select avg(rental_rate) as 대여요금 from film);

-- 서브쿼리 실행 결과가 여러개일 때 in, any, all 을 사용해야 함
...
in (select fc.film_id from film_category fc
	where fc.category_id in (select c.category_id from category c where c.name in ('Comedy', 'Action');

select fc.film_id, f.title from film f
